...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.codexium/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// Directory containing all Codex state (defaults to `~/.codexium` but can be
    ...
    /// Settings that govern if and what will be written to `~/.codexium/history.jsonl`.
    pub history: History,
...
impl Config {
    ...
    /// ignores user settings. For example, the `codexium exec` subcommand is
